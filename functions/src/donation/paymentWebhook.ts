...
import { getItemRefsFromTransactionRef } from '../service/firestoreHelpers';
...
import type { PaymentStatus } from '../../../shared/types/schema';
...
const donationPaymentWebhookHandler = apiMethod.post(hasRequiredBody(
  ...
  async ({ body }, res) => {
    try {
      ...
      const transactionRef = firestore.collections.Transactions.doc(order_id);

      await firestore.db.runTransaction(async (fbt) => {
        if (paymentSummary.status === 'accepted') {
          ...
          const donationRef = firestore.collections.Donations(programRef).doc(order_id);

          await addProgramDonationProgress(donationRef);
          // set donation document visibility by restoring the document
          fbt.update(donationRef, firestore.factory.restoreAttrs());
        }
